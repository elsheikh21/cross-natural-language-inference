    ...
    def encode_dataset(self):
        ...
        for lang_, premise_, hypothesis_, labels_ in tqdm(
            ...
            encoded_dict = tokenizer.encode_plus(text=premise_,
                                                 ...
                                                 return_tensors='pt')
            self.encoded_data.append({'languages': torch.LongTensor(
...
if __name__ == "__main__":
    ...
    model_name_ = "xlm-mlm-xnli15-1024"
    ...
    train_dataset_ = DataLoader(dataset=train_dataset, batch_size=batch_size,
                                collate_fn=XLMDatasetParser.pad_batch,
                                shuffle=True)
    ...
    model = XLM_Model(hp, model_name_).to(device_)
    model.print_summary()

    log_path = os.path.join(os.getcwd(), 'runs', hp.model_name)
    writer_ = WriterTensorboardX(log_path, logger=logging, enable=True)
    optimizer_ = Adam(model.parameters(), lr=5e-6)
    epochs_ = 1

    trainer = XLMTrainer(model=model, writer=writer_,
    ...
    for sample in tqdm(test_dataset_, desc='Predicting on Testing dataset',
        ...
        languages = sample["languages"].to(device_)
        ...
        batch_predicted_labels = model.predict_sentence_(languages, seq, mask, token_types)
